...
// style the source link button dynamically
  let sourceBtnClass
  if (props.idea.source === 'Twitter thread') {
    sourceBtnClass = 'btn-info'
  } else if (props.idea.source === 'My First Million podcast') {
    sourceBtnClass = 'btn-warning'
  } else if (props.idea.source === 'RFS 100 newsletter') {
  ...
  return (
    <div className="card w-75 box-shadow mb-5">
      <div className="card-body mx-xl-5 mx-lg-5 mx-md-2 mx-sm-1 mx-xs-1 py-xl-3 py-lg-3 my-xl-3 my-lg-3">
        ...
        <a href={props.idea.link} target="_blank" rel="noopener noreferrer" className={`btn ${sourceBtnClass} border-0 py-2 px-2`} role="button">{props.idea.source}</a><br></br><br></br>
